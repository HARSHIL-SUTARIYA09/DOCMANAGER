...
if st.session_state.show_reset:
    ...
    if st.button("Confirm Reset"):
        if password_input == ADMIN_PASSWORD:
            ...
            import shutil # shutil works with os
...
tabs = st.tabs(["Uploads ", "Search & View", "Analytics"])

with tabs[0]:
    st.header("Upload PDF")

    uploaded_file = st.file_uploader("Upload PDF", type=["pdf"]) # file uploder widget only with pdf supported.
    tags = st.text_input("Tags(comma separated)") # text_input = to take input from user. and saying that provide it in comma separetd.
    description = st.text_area("Description") # text_area = create area to write and also can add height to make it big.
    lecture_date = st.date_input("Lecture Date (optional)", value=None) # date_input = it enable selecting the date from UI. value=None  ==  it wiont give any default value right now.
...
with tabs[1]:
    ...
    if results and not st.session_state.reader_mode:
        ...
        with container:
            for doc in results:
                # doc is object of Document from models.py
                col1,col2 = st.columns([1, 3])
                # 1+3 = 4 --> 25% left, 75% right
    ...
    if st.session_state.reader_mode and st.session_state.selected_doc:
        ...
        if not os.path.exists(image_dir):
            st.error("Images not found. PDF con=version failed.")
        else:
